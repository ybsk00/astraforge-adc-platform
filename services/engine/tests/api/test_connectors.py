...
class TestConnectorRun:
    ...
    def test_run_connector(self, client):
        ...
        response = client.post(
            "/api/v1/connectors/pubmed/run",
            json={"query": "ADC cancer", "limit": 50}
        )
...
class TestIngestionAPI:
    ...
    def client(self):
        with patch('app.api.ingestion.get_client') as mock_get_client:
            ...
            with patch.dict('sys.modules', {'app.db': MagicMock(), 'app.db.supabase': MagicMock()}):
                from app.main import app
                yield TestClient(app)
