...
async def verify_staging_flow():
    ...
    try:
        ...
        pending = db.table("staging_components").select("status").eq(
        ...
        # 승인 확인
        approved = db.table("staging_components").select("status").eq(
            "id", test_component_id
        ).execute()
        ...
        if catalog_component_id:
            db.table("component_catalog").delete().eq("id", catalog_component_id).execute()
        if test_component_id:
            db.table("staging_components").delete().eq("id", test_component_id).execute()
