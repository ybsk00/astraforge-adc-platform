...
async def run_all_verifications():
    ...
    try:
        ...
        try:
            profiles = db.table("target_profiles").select("id", count="exact").execute()
        ...
        try:
            db.table("staging_components").select("id").limit(1).execute()
            db.table("component_catalog").select("id").limit(1).execute()
        ...
        try:
            ...
            cursors = (
                db.table("ingestion_cursors").select("id", count="exact").execute()
            )
            ...
            dod_results["observability"]["details"] = (
                f"{log_count} logs, {cursor_count} cursors"
            )
