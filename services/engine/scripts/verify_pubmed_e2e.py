...
async def verify_pubmed_e2e():
    ...
    try:
        ...
        # ADC 관련 검색어로 100편 수집
        seed = {"query": "antibody drug conjugate ADC", "retmax": 100}
        ...
        results["fetch"]["status"] = (
            "success" if results["fetch"]["count"] > 0 else "failed"
        )
        ...
        results["raw_save"]["status"] = (
            "success" if results["raw_save"]["count"] > 0 else "failed"
        )
        ...
        try:
            chunks = db.table("literature_chunks").select("id", count="exact").execute()
            results["chunk"]["count"] = chunks.count or 0
            results["chunk"]["status"] = (
                "success" if results["chunk"]["count"] > 0 else "pending"
            )
        ...
        try:
            ...
            results["embed"]["status"] = (
                "success" if results["embed"]["count"] > 0 else "pending"
            )
    ...
    print("=" * 60)
    print(
        f"  Fetch:     {results['fetch']['status']:10} ({results['fetch']['count']} records)"
    )
    print(
        f"  Raw Save:  {results['raw_save']['status']:10} ({results['raw_save']['count']} records)"
    )
    print(f"  Normalize: {results['normalize']['status']:10}")
    print(
        f"  Chunk:     {results['chunk']['status']:10} ({results['chunk']['count']} chunks)"
    )
    print(
        f"  Embed:     {results['embed']['status']:10} ({results['embed']['count']} embeddings)"
    )
