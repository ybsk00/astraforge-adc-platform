...
async def seed_golden_set():
    ...
    for c in cands:
        if c["drug_name"] == "Kadcyla":
            measurements.extend([
                {"candidate_id": c["id"], "metric_name": "IC50", "value": 0.5, "unit": "nM", "assay_type": "cell_line"},
                {"candidate_id": c["id"], "metric_name": "Aggregation_pct", "value": 1.2, "unit": "%", "assay_type": "sec"}
            ])
        elif c["drug_name"] == "Adcetris":
            measurements.extend([
                {"candidate_id": c["id"], "metric_name": "IC50", "value": 2.1, "unit": "nM", "assay_type": "cell_line"},
                {"candidate_id": c["id"], "metric_name": "Aggregation_pct", "value": 0.8, "unit": "%", "assay_type": "sec"}
            ])
