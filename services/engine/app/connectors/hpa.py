...
class HPAConnector(BaseConnector):
    ...
    async def build_queries(self, seed: Dict[str, Any]) -> List[QuerySpec]:
        ...
        for eid in ensembl_ids:
            queries.append(QuerySpec(query=eid, params={"type": "ensembl"}))

        for gene in gene_symbols:
            queries.append(QuerySpec(query=gene, params={"type": "gene"}))
    ...
    async def fetch_page(self, query: QuerySpec, cursor: CursorState) -> FetchResult:
        ...
        try:
            ...
            return FetchResult(records=[data] if data else [], has_more=False)
