...
class CursorState:
    ...
    stats: Dict[str, int] = field(
        default_factory=lambda: {"fetched": 0, "new": 0, "updated": 0, "errors": 0}
    )
...
class BaseConnector(ABC):
    ...
    async def save_raw_data(
        ...
        for item in raw_items:
            ...
            source_id = str(
                item.get("id") or item.get("nctId") or item.get("pmid") or "unknown"
            )
        ...
        try:
            ...
            res = (
                ...
                .upsert(rows, on_conflict="source,source_hash", ignore_duplicates=True)
                .execute()
            )
    ...
    async def run(
        ...
        try:
            ...
            for query in queries:
                page_count = 0
                current_cursor = cursor or CursorState(cursor_id="", source=self.source)
