...
class UniProtConnector(BaseConnector):
    ...
    async def upsert(self, records: List[NormalizedRecord]) -> UpsertResult:
        ...
        for record in records:
            try:
                ...
                result.errors += 1
                self.logger.warning("upsert_failed", 
                                   uniprot_id=record.external_id, 
                                   error=str(e))
    ...
    async def _upsert_target_profile(self, record: NormalizedRecord) -> str:
        ...
        # 기존 프로필 확인
        existing = self.db.table("target_profiles").select("id, checksum").eq(
...
async def enrich_targets_from_catalog(db, connector: UniProtConnector):
    ...
    result = db.table("component_catalog").select("id, name, properties").eq(
        "type", "target"
