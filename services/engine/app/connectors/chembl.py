...
class ChEMBLConnector(BaseConnector):
    ...
    async def build_queries(self, seed: Dict[str, Any]) -> List[QuerySpec]:
        ...
        if not profile_name:
            ...
            for cid in chembl_ids:
                queries.append(QuerySpec(query=cid, params={"type": "molecule"}))

            if smiles:
                queries.append(QuerySpec(query=smiles, params={"type": "smiles"}))

            if search:
                queries.append(QuerySpec(query=search, params={"type": "search"}))
        ...
        if profile.get("mode") == "expansion":
            ...
            for smi in smiles_list:
                queries.append(
                    QuerySpec(
                        query=smi, params={"type": "similarity", "threshold": threshold}
                    )
                )
        else:
    ...
    async def fetch_page(self, query: QuerySpec, cursor: CursorState) -> FetchResult:
        ...
        if query_type == "molecule":
            ...
            return await self._fetch_by_similarity(
                query.query, query.params.get("threshold", 70), offset, limit
            )
        else:
    ...
    async def _fetch_molecule(self, chembl_id: str) -> FetchResult:
        ...
        try:
            response = await fetch_with_retry(
                url, rate_limiter=self.rate_limiter, max_retries=self.max_retries
            )
    ...
    async def _fetch_activities(
        self, chembl_id: str, limit: int = 50
    ) -> Dict[str, Any]:
        """활성 데이터 조회"""
        ...
        try:
            ...
            for act in activities:
                ...
                summary["assay_types"][assay_type] = (
                    summary["assay_types"].get(assay_type, 0) + 1
                )
    ...
    async def _fetch_by_smiles(self, smiles: str) -> FetchResult:
        ...
        return FetchResult(records=molecules, has_more=False)

    async def _fetch_by_similarity(
        self, smiles: str, threshold: int, offset: int, limit: int
    ) -> FetchResult:
        ...
        self.logger.info(
            "chembl_similarity_search", smiles=smiles[:30], threshold=threshold
        )
        ...
        return FetchResult(
            records=molecules, has_more=has_more, next_cursor={"offset": offset + limit}
        )
