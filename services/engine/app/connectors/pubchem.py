...
class PubChemConnector(BaseConnector):
    ...
    async def build_queries(self, seed: Dict[str, Any]) -> List[QuerySpec]:
        ...
        if not profile_name:
            ...
            for i in range(0, len(cids), 100):
                batch = cids[i : i + 100]
                queries.append(
                    QuerySpec(query=",".join(map(str, batch)), params={"type": "cid"})
                )

            # InChIKey (개별)
            for key in inchi_keys:
                queries.append(QuerySpec(query=key, params={"type": "inchikey"}))

            # SMILES
            if isinstance(smiles_list, str):
                smiles_list = [smiles_list]
            for smi in smiles_list:
                queries.append(QuerySpec(query=smi, params={"type": "smiles"}))

            # Names
            for name in names:
                queries.append(QuerySpec(query=name, params={"type": "name"}))
        ...
        if profile.get("mode") == "enrichment":
            ...
            for payload in payloads:
                queries.append(QuerySpec(query=payload, params={"type": "name"}))
        else:
    ...
    async def _fetch_by_cid(self, cids: str) -> FetchResult:
        ...
        try:
            ...
            return FetchResult(records=properties, has_more=False)
    ...
    async def _fetch_synonyms(self, cid: int, limit: int = 10) -> List[str]:
        ...
        try:
            response = await fetch_with_retry(
                url, rate_limiter=self.rate_limiter, max_retries=2
            )
