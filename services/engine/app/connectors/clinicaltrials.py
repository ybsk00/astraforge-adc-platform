...
class ClinicalTrialsConnector(BaseConnector):
    ...
    async def build_queries(self, seed: Dict[str, Any]) -> List[QuerySpec]:
        ...
        for target in targets:
            ...
            # 검색어 구성을 위해 따옴표 처리
            synonyms_str = " OR ".join(
                [f'"{s}"' if " " in s else s for s in synonyms_list]
            )
    ...
    async def fetch_page(self, query: QuerySpec, cursor: CursorState) -> FetchResult:
        ...
        try:
            ...
            return FetchResult(
                ...
                next_cursor={"nextPageToken": next_page_token}
                if next_page_token
                else {},
            )
    ...
    async def _update_target_clinical(self, record: NormalizedRecord) -> str:
        ...
        for cond in conditions:
            ...
            if any(
                keyword in cond_lower
                for keyword in ["her2", "egfr", "pd-1", "pd-l1", "bcma", "cd19", "cd20"]
            ):
