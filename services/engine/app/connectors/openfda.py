...
class OpenFDAConnector(BaseConnector):
    ...
    async def build_queries(self, seed: Dict[str, Any]) -> List[QuerySpec]:
        ...
        for name in drug_names:
            queries.append(QuerySpec(
                query=name,
                params={"type": "drug_name"}
            ))
        
        for name in generic_names:
            queries.append(QuerySpec(
                query=name,
                params={"type": "generic_name"}
            ))
        
        for name in brand_names:
            queries.append(QuerySpec(
                query=name,
                params={"type": "brand_name"}
            ))
    ...
    async def fetch_page(
        ...
        self.logger.info("openfda_request", query=query.query, type=query_type, skip=skip)
            ...
            try:
                ...
                if drug_name:
                    ...
                    for reaction in record.data.get("reactions", []):
                        ...
                        signals["reactions"][term] = signals["reactions"].get(term, 0) + 1
    ...
    async def _update_safety_signals(self, drug_name: str, signals: Dict):
        ...
        if related_compound_id:
            components = self.db.table("component_catalog").select("id, properties").eq(
                "type", "payload"
            ).execute()
        ...
        self.logger.info("safety_signals_aggregated", 
                        drug=drug_name, 
...
async def aggregate_safety_signals(db, drug_name: str) -> Dict:
    ...
    records = db.table("raw_source_records").select("payload").eq(
        "source", "openfda"
    ).execute()
    ...
    for record in records.data:
        ...
        if len(receive_date) >= 4:
            year = receive_date[:4]
            signals["reports_by_year"][year] = signals["reports_by_year"].get(year, 0) + 1
