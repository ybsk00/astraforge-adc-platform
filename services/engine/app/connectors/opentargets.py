...
class OpenTargetsConnector(BaseConnector):
    ...
    async def build_queries(self, seed: Dict[str, Any]) -> List[QuerySpec]:
        ...
        if seed_set_id and self.db:
            ...
            targets_res = (
                self.db.table("seed_set_targets")
                .select("entity_targets(ensembl_gene_id)")
                .eq("seed_set_id", seed_set_id)
                .execute()
            )

            ensembl_ids = [
                t["entity_targets"]["ensembl_gene_id"]
                for t in targets_res.data
                if t.get("entity_targets")
                and t["entity_targets"].get("ensembl_gene_id")
            ]

            for eid in ensembl_ids:
                queries.append(QuerySpec(query=eid, params={"type": "associations"}))

            self.logger.info(
                "seed_set_queries_built", count=len(queries), seed_set_id=seed_set_id
            )
            return queries
    ...
    async def fetch_page(self, query: QuerySpec, cursor: CursorState) -> FetchResult:
        ...
        variables = {"ensemblId": ensembl_id, "page": {"index": page, "size": size}}
        ...
        return FetchResult(
            records=records, has_more=has_more, next_cursor={"page": page + 1}
        )
    ...
    def normalize(self, record: Dict[str, Any]) -> Optional[NormalizedRecord]:
        ...
        top_associations = []
        for assoc in sorted(
            associations, key=lambda x: x.get("score", 0), reverse=True
        )[:20]:
    ...
    async def upsert(self, records: List[NormalizedRecord]) -> UpsertResult:
        ...
        for record in records:
            try:
                ...
                result.errors += 1
                self.logger.warning(
                    "upsert_failed", ensembl_id=record.external_id, error=str(e)
                )
