...
class PipelineService:
    ...
    async def run_seed_set(
        ...
        target_connectors = (
            connector_names if connector_names else list(connector_map.keys())
        )
        ...
        for name in target_connectors:
            ...
            try:
                ...
                run_result = await connector.run(
                    seed={"seed_set_id": seed_set_id}, max_pages=max_pages
                )
                ...
                self.logger.error(
                    "connector_execution_failed", connector=name, error=str(e)
                )
