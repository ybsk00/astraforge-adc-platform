...
class ParetoService:
    ...
    def calculate_pareto_fronts(self, candidates: List[Dict[str, Any]], dimensions: List[str]) -> List[List[str]]:
    ...
    def _dominates(self, p: Dict[str, Any], q: Dict[str, Any], dimensions: List[str]) -> bool:
    ...
    async def save_pareto_results(self, run_id: str, fronts: List[List[str]], dimensions: List[str]):
        """파레토 결과를 DB에 저장"""
        for i, member_ids in enumerate(fronts):
            # 1. Front 정보 저장
            front_res = await self.db.table("run_pareto_fronts").insert({
                "run_id": run_id,
                "front_index": i,
            ...
            # 2. Member 정보 저장
            members = [{"pareto_front_id": front_id, "candidate_id": mid} for mid in member_ids]
            if members:
