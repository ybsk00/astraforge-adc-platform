...
class Orchestrator:
    ...
    async def run_design_pipeline(self, run_id: UUID):
        ...
        try:
            ...
            await self._update_progress(run_id, 2, "Generation", "completed", f"Generated {len(candidates)} candidates")
            ...
            score_results = await self.scoring_service.calculate_scores(str(run_id), candidates)
            # 점수 저장 (디테일 생략, MVP 수준)
            await self._update_progress(run_id, 4, "Scoring", "completed", f"Scored {len(score_results)} candidates")
            ...
            fronts = self.pareto_service.calculate_pareto_fronts(score_dicts, dimensions)
            await self.pareto_service.save_pareto_results(str(run_id), fronts, dimensions)
            await self._update_progress(run_id, 5, "Pareto", "completed", f"Found {len(fronts)} fronts")
            ...
            for cand_id in top_candidate_ids[:5]:  # MVP: 상위 5개만
                ...
                score_comp = next(s for s in score_results if s.candidate_id == cand_id).components
                evidence = await self.evidence_service.generate_evidence(cand_data, score_comp)
                # DB 저장
                await self.db.table("evidence_signals").insert(
                    self.evidence_service.to_db_format(cand_id, evidence)
                ).execute()
            ...
            for cand_id in top_candidate_ids[:5]:
                score_comp = next(s for s in score_results if s.candidate_id == cand_id).components
                protocols = self.protocol_service.generate_protocols(cand_id, score_comp)
                for p in protocols:
                    await self.db.table("design_protocols").insert(
                        self.protocol_service.to_db_format(p)
                    ).execute()
            ...
            await self._update_progress(run_id, 9, "Reporting", "completed", f"Report generated at {report_path}")
            
            self.logger.info("design_pipeline_completed", run_id=str(run_id))

        except Exception as e:
            self.logger.error("design_pipeline_failed", run_id=str(run_id), error=str(e))
            # 장애 상태 관리 (Degraded)
            await self._update_progress(run_id, 0, "Error", "failed", f"System Degraded: {str(e)}")
            await self.audit_service.log_event("RUN_FAILED", resource_id=str(run_id), metadata={"error": str(e)})
            raise

    async def _update_progress(self, run_id: UUID, step: int, phase: str, status: str, message: str = None):
        """런 진행률 업데이트"""
        data = {
            "run_id": str(run_id),
            "step_number": step,
    ...
    async def _generate_candidates(self, run_id: UUID) -> List[Dict[str, Any]]:
        ...
        res = await self.db.table("design_candidates").select("*").eq("run_id", str(run_id)).execute()
        return res.data

    async def _evaluate_appropriateness(self, run_id: UUID, candidates: List[Dict[str, Any]]):
        """적절성 평가 실행 (CalcEngine)"""
        for cand in candidates:
            # 1. Payload RDKit Calculation
            payload_smiles = cand.get("payload_smiles")
            if payload_smiles:
                res = self.calc_engine.calculate_payload(cand["payload_id"], payload_smiles)
                if res:
                    await self.db.table("computations_payload_rdkit").insert({
                        "run_id": str(run_id),
