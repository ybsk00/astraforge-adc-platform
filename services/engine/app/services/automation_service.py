...
class AutomationService:
    ...
    async def handle_scoring_param_change(self, payload: Dict[str, Any]):
        ...
        self.logger.info("scoring_param_change_detected", 
                         param_id=param_id, 
                         version=version, 
                         is_active=is_active)
        ...
        if is_active:
            try:
                ...
                self.logger.info("auto_validation_completed", 
                                 version=version, 
                ...
                self.logger.error("auto_validation_failed", 
                                  version=version, 
                                  error=str(e))
                # 실패 로그 기록
                await self.audit.log_event(
            ...
            self.logger.info("auto_validation_skipped", version=version, reason="not_active")
            return None
