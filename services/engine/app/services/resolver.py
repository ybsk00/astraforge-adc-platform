...
class ResolverService:
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                response = await client.post(
                    self.OT_API_URL, json={"query": query, "variables": variables}
                )
                ...
                # 첫 번째 검색 결과 사용 (정확도 순)
                best_match = hits[0]
                return {"ensembl_gene_id": best_match["id"], "gene_symbol": gene_symbol}
        except Exception as e:
            self.logger.error(
                "resolve_target_failed", gene_symbol=gene_symbol, error=str(e)
            )
            return None
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                return {
                    ...
                    "ontology_source": "EFO"
                    if best_match["id"].startswith("EFO")
                    else "MONDO",
                    "disease_name": disease_name,
                }
        except Exception as e:
            self.logger.error(
                "resolve_disease_failed", disease_name=disease_name, error=str(e)
            )
            return None
    ...
    async def resolve_seed_set(self, seed_set_id: str) -> Dict[str, Any]:
                ...
                if target and not target.get("ensembl_gene_id"):
                    ...
                    if resolved:
                        ...
                        self.logger.info(
                            ...
                            ensembl_id=resolved["ensembl_gene_id"],
                        )
        except Exception as e:
        ...
        try:
            diseases_res = (
                self.db.table("seed_set_diseases")
                .select("entity_diseases(*)")
                .eq("seed_set_id", seed_set_id)
                .execute()
            )
        ...
        return {"status": "completed", "stats": stats, "seed_set_id": seed_set_id}
