...
class ChunkingService:
    ...
    def chunk_document(
        ...
        for text in title_chunks:
            chunks.append(self._create_chunk(
                document_id, chunk_index, text, "abstract"
            ))
            chunk_index += 1
        ...
        if full_text:
            ...
            for section_name, section_text in sections.items():
                ...
                for text in section_chunks:
                    chunks.append(self._create_chunk(
                        document_id, chunk_index, text, section_name
                    ))
                    chunk_index += 1
    ...
    def detect_polarity(self, chunk: Chunk) -> str:
        ...
        # Negative 신호
        negative_keywords = [
            "failed", "failure", "discontinued", "terminated", "toxicity",
            "adverse", "death", "mortality", "withdrew", "withdrawn",
            "ineffective", "no significant", "did not show"
        ]
        
        # Positive 신호
        positive_keywords = [
            "effective", "success", "approved", "improved", "benefit",
            "significant improvement", "response rate", "well tolerated"
        ]
            ...
            if self.db:
                for chunk_data in embedded_chunks:
                    try:
                        ...
                        }, on_conflict="document_id,chunk_index").execute()
                    except Exception as e:
