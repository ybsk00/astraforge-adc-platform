...
class PayloadCalcResult:
    ...
    rationale: Dict[str, List[str]] = field(default_factory=lambda: {"aggregation": [], "bystander": []})
...
class CalcEngine:
    ...
    def calculate_payload(self, payload_id: str, smiles: str) -> Optional[PayloadCalcResult]:
        """페이로드 물성 및 점수 계산"""
        ...
        res.aggregation_score, res.rationale["aggregation"] = self._calc_aggregation_score(res)
        
        # 3. Bystander Proxy Score (0~100)
        res.bystander_proxy_score, res.rationale["bystander"] = self._calc_bystander_score(res)
    ...
    def _calc_aggregation_score(self, res: PayloadCalcResult) -> tuple[float, List[str]]:
        """응집성 점수 산출 (설계서 규칙 반영)"""
        score = 0.0
        rationale = []
        ...
        if res.fsp3 > 0.35:
            score -= 10
            rationale.append(f"High Fsp3 ({res.fsp3:.2f}) > 0.35: -10 (Risk Mitigation)")
    ...
    def calculate_bsab_applicability(
        ...
        # 최종 점수
        res.bsab_applicability_score = max(0.0, min(100.0, 
            sel_score + int_score - co_risk - saf_score
        ))
