...
class EvidenceRAGService:
    ...
    async def _search_literature(
        ...
        try:
            # 키워드 검색 (tsvector)
            keyword_result = self.db.table("literature_chunks").select(
                "*, literature_documents(pmid, title)"
            ).text_search("tsvector_content", query).limit(top_k // 2).execute()
    ...
    def _check_conflict(
        ...
        if pos_count > 0 and neg_count > 0:
            ...
            if ratio > self.CONFLICT_THRESHOLD:
                return True, f"Conflicting evidence: {pos_count} positive, {neg_count} negative citations"
