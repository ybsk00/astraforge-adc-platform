...
class ReportService:
    ...
    async def generate_report_with_cache(self, run_id: UUID, versions: Dict[str, str]) -> str:
        ...
        self.logger.info("report_generation_started", run_id=str(run_id), cache_key=cache_key)
        
        # 2. 캐시 확인
        cached = await self.db.table("report_cache").select("*").eq("cache_key", cache_key).execute()
        ...
        # 3. 동시성 제어 (선점 시도)
        try:
            await self.db.table("report_cache").insert({
                "run_id": str(run_id),
                "cache_key": cache_key,
        ...
        await self.db.table("report_cache").update({
            "bytes": file_size
    ...
    async def _get_signed_url(self, object_path: str) -> str:
        """7일 유효한 서명된 URL 발급"""
        res = await self.db.storage.from_("reports").create_signed_url(object_path, 604800)
        return res["signedURL"]

    async def _fetch_run_data(self, run_id: UUID) -> Dict[str, Any]:
        res = await self.db.table("design_runs").select("*").eq("id", str(run_id)).single().execute()
        return res.data

    async def _fetch_top_candidates(self, run_id: UUID) -> List[Dict[str, Any]]:
        res = await self.db.table("run_pareto_members")\
        ...
        cand_res = await self.db.table("design_candidates").select("*").in_("id", cand_ids).execute()
        return cand_res.data

    async def _fetch_evidence(self, run_id: UUID) -> List[Dict[str, Any]]:
        res = await self.db.table("evidence_signals")\
            .select("*, design_candidates!inner(run_id)")\
            .eq("design_candidates.run_id", str(run_id))\
            .execute()
        return res.data

    def _render_html(self, run_data: Dict[str, Any], candidates: List[Dict[str, Any]], evidence: List[Dict[str, Any]]) -> str:
        template_str = """
        <html>
        <head>
            <style>
