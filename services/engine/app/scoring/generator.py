...
class HardRejectFilter:
    ...
    def check(
        ...
        # 2. 비활성 컴포넌트
        for name, comp in [("target", target), ("antibody", antibody), 
                          ("linker", linker), ("payload", payload)]:
        ...
        if linker_type == "cleavable" and payload_class == "non_cleavable_only":
            return True, "LINKER_PAYLOAD_INCOMPATIBLE", "Cleavable linker with non-cleavable payload"
        
        # 4. Target 발현량 임계값
        tumor_expr = target.get("expression", {}).get("tumor", 0)
        if tumor_expr < 1.0:  # 최소 발현량
            return True, "LOW_TARGET_EXPRESSION", f"Target expression too low: {tumor_expr}"
        
        # 5. ADC 제외 목록
        if target.get("adc_excluded"):
            return True, "ADC_EXCLUDED_TARGET", "Target is excluded from ADC development"
        
        # 6. 커스텀 룰 체크
        for rule in self.rules:
            if rule.get("action") == "hard_reject":
                if self._evaluate_rule(rule, target, antibody, linker, payload):
                    return True, rule.get("id", "CUSTOM_RULE"), rule.get("reason", "Custom rule violation")
...
def create_generator_from_catalog(
    ...
    def load_components(comp_type: str, ids: List[str] = None) -> List[Dict[str, Any]]:
        query = db_client.table("component_catalog").select("*").eq("type", comp_type).eq("status", "active")
