    ...
    def score_to_dict(self, scores: CandidateScores) -> Dict[str, Any]:
        ...
        return {
            ...
            "score_components": {
                "eng_fit": {
                    "terms": {
                        k: round(v, 2) for k, v in scores.eng_components.terms.items()
                    },
                ...
                "bio_fit": {
                    "terms": {
                        k: round(v, 2) for k, v in scores.bio_components.terms.items()
                    },
                ...
                "safety_fit": {
                    "terms": {
                        k: round(v, 2)
                        for k, v in scores.safety_components.terms.items()
                    },
...
class BatchScoringEngine(ScoringEngine):
    ...
    def score_batch(self, candidates: List[Dict[str, Any]]) -> List[CandidateScores]:
        """
        배치 스코어링
