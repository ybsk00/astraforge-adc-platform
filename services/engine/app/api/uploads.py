...
async def presign_upload(request: PresignRequest, db=Depends(get_db)):
    ...
    # Storage key 생성
    storage_key = (
        f"users/{request.user_id}/{year}/{month}/{upload_id}/{request.filename}"
    )
    bucket = "user-uploads"
    ...
    try:
        ...
        try:
            signed_url_result = db.storage.from_(bucket).create_signed_upload_url(
                storage_key
            )
            ...
            if not presigned_url:
                ...
                supabase_url = os.getenv("SUPABASE_URL", "")
                presigned_url = (
                    f"{supabase_url}/storage/v1/object/{bucket}/{storage_key}"
                )
...
async def commit_upload(request: CommitRequest, db=Depends(get_db)):
    ...
    try:
        ...
        result = (
            ...
            .eq("owner_user_id", request.user_id)
            .execute()
        )

        if not result.data:
            raise HTTPException(
                status_code=404, detail="Upload not found or access denied"
            )
        ...
        if upload["status"] != "uploaded":
            raise HTTPException(
                status_code=400, detail=f"Invalid upload status: {upload['status']}"
            )

        # 2. status 변경
        db.table("uploads").update({"status": "parsing"}).eq(
