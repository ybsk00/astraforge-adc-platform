...
class StagingComponentCreate(BaseModel):
    ...
    type: str = Field(
        ..., description="컴포넌트 타입 (target/antibody/linker/payload/conjugation)"
    )
...
async def create_staging_component(data: StagingComponentCreate):
    ...
    try:
        ...
        if not result.data:
            raise HTTPException(
                status_code=500, detail="Failed to create staging component"
            )

        component = result.data[0]
        logger.info(
            "staging_component_created",
            id=component["id"],
            type=data.type,
            name=data.name,
        )
...
async def update_staging_component(component_id: UUID, data: StagingComponentUpdate):
    ...
    try:
        ...
        if existing.data[0]["status"] != "pending":
            raise HTTPException(
                status_code=400, detail="Cannot modify non-pending component"
            )
...
async def approve_staging_component(component_id: UUID, data: StagingApproval = None):
    ...
    try:
        ...
        if not catalog_result.data:
            raise HTTPException(
                status_code=500, detail="Failed to create catalog component"
            )
...
async def bulk_approve_staging_components(data: BulkApproval):
    ...
    for component_id in data.ids:
        try:
            await approve_staging_component(
                UUID(component_id), StagingApproval(review_note=data.review_note)
            )
            results["approved"] += 1
        except Exception as e:
            results["failed"] += 1
    ...
    logger.info(
        "bulk_approve_completed", approved=results["approved"], failed=results["failed"]
    )
...
async def get_duplicate_groups(
    ...
    try:
        # pending 상태만 조회
        result = (
            db.table("staging_components").select("*").eq("status", "pending").execute()
        )
...
async def get_staging_stats():
    ...
    try:
        ...
        stats = {"total": len(result.data), "by_status": {}, "by_type": {}}
