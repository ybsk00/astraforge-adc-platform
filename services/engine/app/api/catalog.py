    ...
    try:
        ...
        if data.properties.get("smiles"):
            try:
                ...
                logger.warning(
                    "enqueue_failed", component_id=component["id"], error=str(e)
                )
...
async def get_component(component_id: UUID):
    ...
    try:
        ...
        logger.error(
            "component_get_failed", component_id=str(component_id), error=str(e)
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_component(component_id: UUID, data: ComponentUpdate):
    ...
    try:
        ...
        if "properties" in update_data and "smiles" in update_data.get(
            "properties", {}
        ):
        ...
        logger.error(
            "component_update_failed", component_id=str(component_id), error=str(e)
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_component(component_id: UUID):
    ...
    try:
        ...
        # deprecated로 변경
        db.table("component_catalog").update({"status": "deprecated"}).eq(
            "id", str(component_id)
        ).execute()
        ...
        logger.error(
            "component_delete_failed", component_id=str(component_id), error=str(e)
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def retry_compute(component_id: UUID):
    ...
    try:
        ...
        if component["status"] != "failed":
            raise HTTPException(
                status_code=400, detail="Only failed components can be retried"
            )

        # status를 pending_compute로 변경
        db.table("component_catalog").update(
            {"status": "pending_compute", "compute_error": None}
        ...
        try:
            ...
            logger.warning(
                "enqueue_failed", component_id=str(component_id), error=str(e)
            )
        ...
        logger.error(
            "component_retry_failed", component_id=str(component_id), error=str(e)
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_catalog_stats():
    ...
    try:
        ...
        stats = {"total": len(result.data), "by_type": {}, "by_status": {}}
