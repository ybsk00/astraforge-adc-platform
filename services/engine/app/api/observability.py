...
async def get_system_health():
    ...
    # Supabase 연결 확인
    try:
        result = db.table("ingestion_logs").select("id").limit(1).execute()
    ...
    try:
        ...
        logs = db.table("ingestion_logs").select("status").gte("created_at", since).execute()
        
        total = len(logs.data)
        failed = sum(1 for l in logs.data if l.get("status") == "failed")
        ...
        if total > 0:
            ...
            health["components"]["connectors"] = {"status": "idle", "note": "No runs in last hour"}
    except Exception as e:
