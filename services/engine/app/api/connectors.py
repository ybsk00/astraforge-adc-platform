...
async def get_connector_detail(source: str):
    ...
    # 모든 커서 조회
    cursors = db.table("ingestion_cursors").select("*").eq("source", source).order(
        "updated_at", desc=True
    ).limit(10).execute()
    
    # 최근 로그 조회
    logs = db.table("ingestion_logs").select("*").eq("source", source).order(
...
async def get_connector_status(source: str):
    ...
    # 가장 최근 커서
    cursor = db.table("ingestion_cursors").select("*").eq("source", source).order(
        "updated_at", desc=True
    ).limit(1).execute()
...
async def run_connector(
    ...
    try:
        # 이미 실행 중인지 확인
        running = db.table("ingestion_cursors").select("id").eq("source", source).eq(
            "status", "running"
        ).execute()
...
async def retry_connector(source: str):
    ...
    try:
        # 실패한 커서 조회
        failed = db.table("ingestion_cursors").select("*").eq("source", source).eq(
            "status", "failed"
...
async def get_connector_logs(
    ...
    try:
        query = db.table("ingestion_logs").select("*", count="exact").eq("source", source)
...
async def get_connector_stats(source: str):
    ...
    try:
        # 전체 로그에서 통계 계산
        logs = db.table("ingestion_logs").select(
...
async def update_connector_config(source: str, config: ConnectorConfigUpdate):
    ...
    try:
        # 기존 커서 또는 새로 생성
        existing = db.table("ingestion_cursors").select("id, config").eq(
            "source", source
        ).order("updated_at", desc=True).limit(1).execute()
