...
async def get_overall_stats():
    ...
    # 전체 로그 조회
    all_logs = db.table("ingestion_logs").select(
        "id, source, status, records_fetched, records_new, created_at"
    ).execute()
    ...
    successful_runs = len([log for log in all_logs.data if log.get("status") == "completed"])
...
async def get_source_stats(source: str):
    ...
    successful_runs = len([log for log in logs.data if log.get("status") == "completed"])
    ...
    durations = [log.get("duration_ms", 0) for log in logs.data if log.get("duration_ms")]
...
async def get_ingestion_history(
    ...
    query = db.table("ingestion_logs").select(
        "source, status, records_fetched, records_new, created_at"
