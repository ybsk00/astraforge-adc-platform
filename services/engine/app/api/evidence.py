...
async def search_evidence(
    ...
    source: Optional[str] = Query(None, description="Filter by source (e.g., pubmed, patent)"),
    db=Depends(get_db)
):
    """
    ...
    try:
        ...
        for row in (result.data or []):
            items.append({
                ...
                "year": int(row.get("document_year")[:4]) if row.get("document_year") else None,
                "abstract": row.get("content"), # Showing chunk content as abstract/snippet
