...
async def list_feedback(
    ...
    """피드백 목록 조회"""
    try:
        query = db.table("human_feedback").select("*").eq(
            "workspace_id", workspace_id
...
async def get_feedback_stats(entity_id: str, db=Depends(get_db)):
    """엔티티별 피드백 통계"""
    try:
        result = db.table("human_feedback").select("feedback_type").eq(
            "entity_id", entity_id
        ).execute()
...
async def list_assay_results(
    ...
    """분석 결과 목록 조회"""
    try:
        query = db.table("assay_results").select("*").eq(
            "workspace_id", workspace_id
        ).order("created_at", desc=True)
...
async def mark_as_outlier(
    ...
    try:
        result = db.table("assay_results").update({
