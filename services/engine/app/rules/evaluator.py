...
class SafeExpressionEvaluator:
    ...
    def evaluate(self, expression: str, context: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if op_str in self.SPECIAL_OPERATORS:
                return self._evaluate_special(field_name, field_value, op_str, value_str)
    ...
    def _evaluate_special(
        ...
        if op == "in":
            ...
            return result, f"{field_name} ({field_value}) in {target_list}" if result else ""
            
        if op == "not_in":
            target_list = self._parse_list(value_str)
            result = field_value not in target_list
            return result, f"{field_name} ({field_value}) not in {target_list}" if result else ""
