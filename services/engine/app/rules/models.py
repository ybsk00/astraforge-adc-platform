...
class Rule:
    ...
    action: RuleAction = field(
        default_factory=lambda: RuleAction(type=ActionType.ALERT)
    )
...
class CandidateFeatures:
    ...
    @classmethod
    def from_candidate(
        cls, candidate: Dict[str, Any], scores: Dict[str, Any] = None
    ) -> "CandidateFeatures":
        """Dict에서 표준 피처 추출"""
        ...
        return cls(
            ...
            critical_tissue_expression=bool(
                props.get("critical_tissue_expression", False)
            ),
...
class RuleSet:
    ...
    def get_sorted_rules(self) -> List[Rule]:
        ...
        return sorted(
            [r for r in self.rules if r.enabled], key=lambda r: (r.priority, r.id)
        )
