...
async def design_run_execute(ctx: Dict[str, Any], run_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 스코어링 파라미터 로드
        scoring_params = {}
        params_result = db.table("scoring_params").select("*").eq("is_active", True).execute()
        ...
        for batch in generator.generate_batches():
            ...
            for candidate, score in zip(batch, scores):
                candidate_data = {
                    ...
                    "score_components": scoring_engine.score_to_dict(score).get("score_components", {})
                }
        ...
        for summary in reject_summaries:
            db.table("candidate_reject_summaries").upsert({
                ...
                "updated_at": datetime.utcnow().isoformat()
            }, on_conflict="run_id,reason_code").execute()
