...
def evaluate_gate_status(seed: dict) -> dict:
    ...
    """
    # 필수 조건 체크
    gate1_target = bool(seed.get("resolved_target_symbol") and seed["resolved_target_symbol"].strip() != '')
    gate2_smiles = bool(seed.get("payload_smiles_standardized")) or seed.get("proxy_smiles_flag", False)
    ...
    bonus_score = (5 if has_nct else 0) + (10 if has_rdkit else 0) + (5 if len(evidence) >= 2 else 0)
...
def execute_quality_gate_job(limit: int = 100) -> dict:
    ...
    result = supabase.table("golden_seed_items") \
        .select("*") \
        .eq("is_final", False) \
        .limit(limit) \
        .execute()
