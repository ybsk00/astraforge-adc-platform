    ...
    try:
        ...
        if seed_set_id:
            ...
            queries = await generate_queries(
                ctx, seed_set_id, connector.get("name", "").lower()
            )

            if (
                connector_type == "api"
                and connector.get("name", "").lower() == "pubmed"
            ):
                from .pubmed_job import pubmed_fetch_job

                total_fetched = 0
                for q in queries[:10]:  # MVP에서는 상위 10개 쿼리만 우선 실행
                    res = await pubmed_fetch_job(
                        ctx, {"query": q["query"], "retmax": 50}
                    )
                ...
                connector_type == "api"
                and connector.get("name", "").lower() == "opentargets"
            ):
                ...
                result_summary = {
                    "message": f"Seed-based ingestion for {connector_type}/{connector.get('name')} not fully implemented"
                }
        ...
        # 3-2. 일반 실행 (직접 실행 또는 스케줄링)
            ...
            if connector_type == "api":
                ...
                if connector_name == "pubmed":
                    ...
                    result_summary = {
                        "message": f"No job implementation for API connector: {connector_name}"
                    }
                ...
                if connector_name == "hpa":
                    ...
                    result_summary = {
                        "message": f"No job implementation for DB connector: {connector_name}"
                    }
                ...
                if connector_name == "seed":
                    ...
                    result_summary = await seed_fetch_job(
                        ctx, connector.get("config", {})
                    )
                elif connector_name == "resolve":
                    from .resolve_job import resolve_fetch_job

                    result_summary = await resolve_fetch_job(
                        ctx, connector.get("config", {})
                    )
                else:
                    result_summary = {
                        "message": f"No job implementation for System connector: {connector_name}"
                    }
                ...
                result_summary = await execute_golden_seed(
                    ctx, run_id, connector.get("config", {})
                )
                ...
                result_summary = await rag_seed_query_job(
                    ctx, run_id, connector.get("config", {})
                )
