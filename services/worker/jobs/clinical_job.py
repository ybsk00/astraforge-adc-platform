...
async def clinicaltrials_fetch_job(ctx, seed: Dict[str, Any]):
    ...
    # 기본 쿼리 설정
    if not seed.get("nct_ids") and not seed.get("conditions") and not seed.get("interventions"):
...
async def openfda_fetch_job(ctx, seed: Dict[str, Any]):
    ...
    # 기본 쿼리 설정
    if not seed.get("drug_names") and not seed.get("generic_names") and not seed.get("brand_names"):
