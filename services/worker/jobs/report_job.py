...
async def report_job(ctx, target_set: str = "golden_candidates"):
    ...
    approved_count = sum(
        1 for c in candidates if c.get("approval_status") == "Approved"
    )
    ...
    top_candidates = sorted(
        candidates, key=lambda x: x.get("confidence_score", 0), reverse=True
    )[:5]
    top_summary = [
        {"name": c["drug_name"], "score": c.get("confidence_score")}
    ...
    report = {
        ...
        "distribution": {
            "targets": dict(
                sorted(targets.items(), key=lambda x: x[1], reverse=True)[:5]
            ),
            "payloads": dict(
                sorted(payloads.items(), key=lambda x: x[1], reverse=True)[:5]
            ),
        },
    }
