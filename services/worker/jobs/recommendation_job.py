...
async def recommendation_job(ctx, target_set: str = "golden_candidates"):
    ...
    policy_res = await db.table("scoring_policies").select("*").eq("is_active", True).limit(1).execute()
    ...
    for cand in candidates:
        try:
            ...
            await db.table(target_set).update({
    ...
    return {"status": "completed", "updated": updated_count, "policy": policy["policy_version"]}
