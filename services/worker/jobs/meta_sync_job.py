...
async def opentargets_fetch_job(ctx, seed: Dict[str, Any]):
    ...
    if seed.get("batch_mode"):
        ...
        targets = (
            ...
            .eq("type", "target")
            .not_.is_("ensembl_gene_id", "null")
            .execute()
        )
        catalog_ids = [
            t["ensembl_gene_id"] for t in targets.data if t.get("ensembl_gene_id")
        ]
        if catalog_ids:
            ...
            if not ensembl_ids and not target_id and not query:
                return {
                    "status": "completed",
                    "message": "No targets with Ensembl ID found in Catalog",
                }
    ...
    query_hash = hashlib.md5(
        f"opentargets:{query_key}:{str(seed)}".encode()
    ).hexdigest()[:16]
    ...
    try:
        ...
        if seed.get("batch_mode"):
            ...
            catalog_symbols = [
                ...
                if t.get("gene_symbol") and not t.get("ensembl_gene_id")
            ]
            ...
            if catalog_ensembls or catalog_symbols:
                ...
                logger.info(
                    "hpa_batch_targets_loaded",
                    ensembls=len(catalog_ensembls),
                    symbols=len(catalog_symbols),
                )
            else:
                ...
                if not seed.get("ensembl_ids") and not seed.get("gene_symbols"):
                    return {
                        "status": "completed",
                        "message": "No targets found in Catalog",
                    }
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            for identifier in identifiers:
                ...
                try:
                    ...
                    logger.warning(
                        "hpa_fetch_failed", identifier=identifier, error=str(e)
                    )
...
async def chembl_fetch_job(ctx, seed: Dict[str, Any]):
    ...
    if seed.get("batch_mode"):
        ...
        compounds = (
            db.table("component_catalog")
            .select("chembl_id")
            .in_("type", ["payload", "linker"])
            .not_.is_("chembl_id", "null")
            .execute()
        )
        ...
        if catalog_ids:
            ...
            if not seed.get("chembl_ids"):
                return {
                    "status": "completed",
                    "message": "No compounds with ChEMBL ID found in Catalog",
                }
            ...
            for cid in chembl_ids:
                ...
                try:
                    ...
                    act_resp = await client.get(
                        act_url, params={"molecule_chembl_id": cid, "limit": 50}
                    )
                    activities = []
...
async def save_chembl_data(db, data):
    ...
    if inchi_key:
        existing = (
            db.table("compound_registry")
            .select("id")
            .eq("inchi_key", inchi_key)
            .execute()
        )
    ...
    if existing and existing.data:
        db.table("compound_registry").update(compound_data).eq(
...
async def pubchem_fetch_job(ctx, seed: Dict[str, Any]):
    ...
    if seed.get("batch_mode"):
        ...
        compounds = (
            db.table("component_catalog")
            .select("pubchem_cid, inchikey")
            .in_("type", ["payload", "linker"])
            .or_("pubchem_cid.neq.null,inchikey.neq.null")
            .execute()
        )

        catalog_cids = [
            c["pubchem_cid"] for c in compounds.data if c.get("pubchem_cid")
        ]
        catalog_inchis = [
            c["inchikey"]
            for c in compounds.data
            if c.get("inchikey") and not c.get("pubchem_cid")
        ]  # CID 우선
        ...
        if catalog_cids or catalog_inchis:
            ...
            logger.info(
                "pubchem_batch_compounds_loaded",
                cids=len(catalog_cids),
                inchis=len(catalog_inchis),
            )
        else:
            ...
            if not seed.get("cids") and not seed.get("inchi_keys"):
                return {
                    "status": "completed",
                    "message": "No compounds with PubChem CID or InChIKey found in Catalog",
                }
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            for key in inchi_keys:
                ...
                try:
                    ...
                    logger.warning(
                        "pubchem_inchikey_failed", inchikey=key, error=str(e)
                    )
                ...
                try:
                    ...
                    if props:
                        ...
                        if syn_resp.status_code == 200:
                            ...
                            info = syn_data.get("InformationList", {}).get(
                                "Information", []
                            )
                            if info:
...
async def save_pubchem_data(db, data):
    ...
    if inchi_key:
        existing = (
            db.table("compound_registry")
            .select("id, checksum, pubchem_cid")
            .eq("inchi_key", inchi_key)
            .execute()
        )
