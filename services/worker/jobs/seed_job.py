...
GOLD_TARGETS = [
    ...
    },
    {
        ...
        "ensembl_gene_id": "ENSG00000184292",
    },
    {
        "name": "Nectin-4",
        "gene_symbol": "NECTIN4",
        "uniprot_accession": "Q96NY8",
        "ensembl_gene_id": "ENSG00000143217",
    },
    {
        "name": "EGFR",
        "gene_symbol": "EGFR",
    ...
    },
    {
        ...
        "ensembl_gene_id": "ENSG00000174059",
    },
    {
        "name": "CD20",
        "gene_symbol": "MS4A1",
...
GOLD_PAYLOADS = [
    {
        "name": "MMAE",
        "inchikey": "FBOQWJKAQSDJHD-UHFFFAOYSA-N",
        "pubchem_cid": "11542250",
    ...
    {
        "name": "DXd",
        "inchikey": "YGYAWVDWJNRERW-UHFFFAOYSA-N",
        "pubchem_cid": "137346536",
        "smiles": "O=C1[C@](O)(CC)C2=C(CO1)C(N3CC4=C5C6=C(CC[C@@H]5NC(CO)=O)C(C)=C(F)C=C6N=C4C3=C2)=O",
    },  # Deruxtecan payload
    {
        "name": "SN-38",
        "inchikey": "UFLHZZXDIJEZRE-UHFFFAOYSA-N",
        "pubchem_cid": "104842",
        "smiles": "CCC1=C2CN3C(=CC4=C(C3=O)COC(=O)[C@@]4(CC)O)C2=NC5=C1C=C(C=C5)O",
    },
    {
        "name": "DM1",
        "inchikey": "OVSQDLIZRRETGV-UHFFFAOYSA-N",
        "pubchem_cid": "5352062",
...
GOLD_LINKERS = [
    ...
    {
        "name": "SMCC",
        "inchikey": "ISWJOCWSPKFRBO-UHFFFAOYSA-N",
        "pubchem_cid": "6364619",
    ...
    try:
        ...
        if json_path.exists():
            try:
                with open(json_path, "r", encoding="utf-8") as f:
                    ...
                    logger.info(
                        "seed_file_loaded", path=str(json_path), count=len(file_targets)
                    )

                    # Merge logic: gene_symbol 기준으로 중복 제거 (파일 데이터 우선)
                    existing_symbols = {
                        t["gene_symbol"] for t in targets if "gene_symbol" in t
                    }

                    for ft in file_targets:
                        if (
                            ft.get("gene_symbol")
                            and ft["gene_symbol"] not in existing_symbols
                        ):
                            targets.append(ft)
                        elif (
                            ft.get("gene_symbol")
                            and ft["gene_symbol"] in existing_symbols
                        ):
        ...
        for item in targets:
            data = {
                ...
                "is_gold": item.get("quality_grade") == "gold"
                or item.get("is_gold", False),
            ...
            if data.get("gene_symbol"):
                ...
                existing = (
                    ...
                    .select("id")
                    .eq("type", "target")
                    .eq("gene_symbol", data["gene_symbol"])
                    .is_("workspace_id", "null")
                    .execute()
                )
                if existing.data:
                    db.table("component_catalog").update(data).eq(
