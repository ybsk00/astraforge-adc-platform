...
async def parse_candidate_csv_job(ctx: Dict[str, Any], upload_id: str, user_id: str):
    ...
    try:
        ...
        # 2. Storage에서 파일 다운로드
        try:
            file_data = db.storage.from_(upload["storage_bucket"]).download(
                upload["storage_key"]
            )
        except Exception as e:
            log.error("file_download_failed", error=str(e))
            await update_upload_status(
                db, upload_id, "failed", f"파일 다운로드 실패: {str(e)}"
            )
        ...
        try:
            ...
            await update_upload_status(
                db, upload_id, "failed", f"CSV 파싱 실패: {str(e)}"
            )
            return {"status": "error", "message": str(e)}
                    ...
                    .insert(
                        {
                            ...
                            "metadata": {
                                ...
                                if k not in FEATURE_COLUMNS and k != "name"
                            },
                            "features": features,
                            ...
                            assay_inserts.append(
                                {
                                    ...
                                    or "00000000-0000-0000-0000-000000000000",  # Fallback if not provided
                                    "candidate_id": candidate_id,
                                    "assay_type": key,
...
async def update_upload_status(
    db, upload_id: str, status: str, error_message: Optional[str] = None
):
    """업로드 상태 업데이트"""
