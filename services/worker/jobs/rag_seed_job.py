...
async def rag_seed_query_job(ctx, run_id: str, config: Dict[str, Any]):
    ...
    try:
        ...
        min_similarity = config.get("retrieval", {}).get(
            "min_similarity", 0.5
        )  # 임계값
        ...
        if seed_query_set_id:
            q_res = (
                db.table("seed_queries")
                .select("query_text")
                .eq("seed_query_set_id", seed_query_set_id)
                .execute()
            )
        ...
        for item in extracted_items[:target_count]:
            ...
            target_id = (
                _get_or_create_entity(db, "entity_targets", item["target"])
                if item["target"]
                else None
            )
            antibody_id = (
                _get_or_create_entity(db, "entity_antibodies", item["antibody"])
                if item["antibody"]
                else None
            )
            linker_id = (
                _get_or_create_entity(db, "entity_linkers", item["linker"])
                if item["linker"]
                else None
            )
            payload_id = (
                _get_or_create_entity(db, "entity_payloads", item["payload"])
                if item["payload"]
                else None
            )
            drug_id = (
                _get_or_create_entity(db, "entity_drugs", item["drug_name"])
                if item["drug_name"]
                else None
            )
            ...
            try:
                ...
                if item_res.data:
                    ...
                    doc_info = (
                        db.table("literature_documents")
                        .select("pmid")
                        .eq("id", chunk_info.data["document_id"])
                        .single()
                        .execute()
                    )
                    ...
                    evidence_data = {
                        ...
                        "entity_id": item_res.data[0][
                            "id"
                        ],  # seed_set_item id를 사용하려면 entity_type을 맞춰야 함.
