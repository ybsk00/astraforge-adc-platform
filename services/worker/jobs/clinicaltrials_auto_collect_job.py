...
def search_adc_trials(query_terms: str = "antibody drug conjugate", max_results: int = 50) -> list:
    """
    ...
    try:
        ...
        for intervention in interventions:
            if intervention.get("type") in ["DRUG", "BIOLOGICAL", "COMBINATION_PRODUCT"]:
                ...
                if name and ("adc" in name.lower() or "conjugate" in name.lower() or 
                           "mab" in name.lower() or "zumab" in name.lower() or 
                           "ximab" in name.lower() or "tumab" in name.lower()):
                    drug_names.append(name)
        
        if not drug_names:
            # 제목에서 약물명 추출 시도
            adc_keywords = ["adc", "conjugate", "vedotin", "deruxtecan", "emtansine", 
                          "govitecan", "tesirine", "ozogamicin", "maytansine"]
...
def save_to_candidates(supabase, candidates: list) -> dict:
    ...
    for candidate in candidates:
        try:
            ...
            existing = supabase.table("golden_candidates") \
                .select("id") \
                .or_(f"source_ref.eq.{candidate['nct_id']},drug_name.ilike.{candidate['drug_name']}") \
                .limit(1) \
                .execute()
...
def execute_clinicaltrials_auto_collect(max_results: int = 50) -> dict:
    ...
    print(f"[clinicaltrials_auto_collect] Searching ClinicalTrials.gov...")
    ...
    for query in search_queries:
        print(f"\n  Searching: {query}")
        studies = search_adc_trials(query, max_results=max_results // len(search_queries))
        
        for study in studies:
            drug_info = extract_drug_info(study)
            if drug_info and drug_info["drug_name"]:
                # 중복 제거
                existing = [c for c in all_candidates if c["nct_id"] == drug_info["nct_id"]]
                if not existing:
                    all_candidates.append(drug_info)
                    print(f"    Found: {drug_info['drug_name']} ({drug_info['nct_id']})")
