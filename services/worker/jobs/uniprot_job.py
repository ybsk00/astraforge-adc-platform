...
async def uniprot_fetch_job(ctx, seed: Dict[str, Any]):
    ...
    if seed.get("batch_mode"):
        ...
        targets = (
            ...
            .eq("type", "target")
            .not_.is_("uniprot_accession", "null")
            .execute()
        )
        catalog_ids = [
            t["uniprot_accession"] for t in targets.data if t.get("uniprot_accession")
        ]
        if catalog_ids:
            ...
            return {
                "status": "completed",
                "message": "No targets with UniProt Accession found in Catalog",
            }
    ...
    query_hash = hashlib.md5(f"uniprot:{query_key}:{str(seed)}".encode()).hexdigest()[
        :16
    ]
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            for item in results:
                ...
                gene_symbol = (
                    genes[0].get("geneName", {}).get("value", "") if genes else None
                )
                ...
                organism = item.get("organism", {}).get(
                ...
                existing = (
                    ...
                    .select("id")
                    .eq("uniprot_id", primary_accession)
                    .execute()
                )

                if existing.data:
                    db.table("target_profiles").update(target_data).eq(
                        "uniprot_id", primary_accession
                    ).execute()
                    updated_count += 1
                else:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                for item in results:
                    ...
                    for comment in comments:
                        if comment.get("commentType") == "FUNCTION":
                            function_summary = comment.get("texts", [])[0].get(
                                "value", ""
                            )
                            break
