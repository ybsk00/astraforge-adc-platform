...
def search_pubchem_by_name(name: str) -> dict | None:
    ...
    try:
        ...
        props = (
            smiles_response.json().get("PropertyTable", {}).get("Properties", [{}])[0]
        )
...
def execute_pubchem_enrich_job(limit: int = 100, force_update: bool = False) -> dict:
    ...
    query = (
        ...
        .select(
            "id, drug_name_canonical, payload_exact_name, payload_smiles_standardized, proxy_smiles_flag, field_verified"
        )
    ...
    for seed in seeds:
        ...
        if pubchem_result and pubchem_result.get("smiles"):
            ...
            # 3. Verified 필드 체크
            if (
                is_field_verified(verified_json, "payload_smiles_standardized")
                and not force_update
            ):
                # 자동 업데이트 금지 → Review Queue로 제안
                print("  → Verified field, adding to Review Queue")
                add_to_review_queue(
                    supabase,
                    seed_id,
                    "enrichment_update",
            ...
            # 5. 실패 시 Proxy 제안
            print("  → PubChem not found, suggesting Proxy")
            add_to_review_queue(
                supabase,
                seed_id,
                "proxy_suggestion",
                {
                    "proxy_smiles_flag": {
                        ...
                        "evidence": [
                            {
                                "type": "note",
                                "id": f"PubChem search failed for: {payload_name}",
                            }
                        ],
                    },
                    "proxy_reference": {
