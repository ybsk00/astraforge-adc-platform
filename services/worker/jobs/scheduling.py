...
async def uniprot_weekly_sync(ctx: Dict[str, Any]):
    ...
    seed = {
        "uniprot_ids": [
            ...
            "P11836",
            "P08637",
            "P20963",  # Blood cancer targets
            "P16422",
            "Q15116",
            "Q9NZQ7",  # Solid tumor / Immune
        ]
    }
...
async def chembl_weekly_sync(ctx: Dict[str, Any]):
    ...
    seed = {"search": "maytansine OR MMAE OR PBD dimer", "limit": 100}
...
async def pubchem_weekly_sync(ctx: Dict[str, Any]):
    ...
    seed = {"names": ["maytansine", "mertansine", "MMAE", "MMAF"]}
...
async def openfda_weekly_sync(ctx: Dict[str, Any]):
    ...
    seed = {"drug_names": ["trastuzumab", "brentuximab", "polatuzumab"]}
    ...
    for compound in pending.data:
        try:
            ...
            if fp_result and descriptors:
                ...
                # 실패 시 상태 변경
                db.table("component_catalog").update({"status": "failed"}).eq(
                    "id", compound["id"]
                ).execute()
