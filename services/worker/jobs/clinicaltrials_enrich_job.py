...
def search_trials_by_target(
    target: str, synonyms: list = None, max_results: int = 20
) -> list:
    """
...
def execute_clinicaltrials_enrich_job(
    limit: int = 100, force_update: bool = False
) -> dict:
    """
    ...
    query = (
        ...
        .select(
            "id, drug_name_canonical, target, resolved_target_symbol, clinical_phase, program_status, clinical_nct_id_primary, evidence_refs, field_verified"
        )
        .or_(
            "clinical_phase.is.null,clinical_phase.eq.,program_status.is.null,program_status.eq."
        )
        .limit(limit)
    )
    ...
    print(
        f"[clinicaltrials_enrich_job] Processing {len(seeds)} seeds without clinical info..."
    )
    ...
    for seed in seeds:
        ...
        if not trial_infos:
            print("  → No valid trial info extracted")
            failed_count += 1
            continue
        ...
        existing_ncts = [
            e.get("id") for e in current_evidence if e.get("type") == "NCT"
        ]
        ...
        if update_data:
            try:
                supabase.table("golden_seed_items").update(update_data).eq(
                    "id", seed_id
                ).execute()
                print(
                    f"  ✓ Updated: phase={update_data.get('clinical_phase', '-')}, status={update_data.get('program_status', '-')}"
                )
                enriched_count += 1
            except Exception as e:
        ...
        if proposed_patch:
            ...
            queued_count += 1
            print("  → Queued verified field updates")
