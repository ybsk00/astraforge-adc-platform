            ...
            for item in targets.data:
                ...
                try:
                    ...
                    if data["results"]:
                        ...
                        logger.info(
                            "target_resolved", name=item["name"], accession=accession
                        )
                    else:
                    ...
                    logger.error(
                        "resolve_target_failed", name=item["name"], error=str(e)
                    )
                    stats["errors"] += 1
                    ...
                    if not item.get("chembl_id"):
                        ...
                        if resp.status_code == 200:
                            ...
                            if data.get("molecules"):
                                ...
                                updates["chembl_id"] = data["molecules"][0][
                                    "molecule_chembl_id"
                                ]
                                logger.info(
                                    "chembl_resolved",
                                    name=name,
                                    chembl_id=updates["chembl_id"],
                                )

                    if updates:
                        updates["updated_at"] = datetime.utcnow().isoformat()
                        db.table("component_catalog").update(updates).eq(
                            "id", item["id"]
                        ).execute()
